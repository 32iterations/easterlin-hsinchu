...
# -*- coding: utf-8 -*-
"""
台積電 ESG 合作提案簡報生成器 V2.0 (16:9 重新設計版)
生成日期: 2025-10-23
...
def create_tsmc_presentation_v2():
    """創建台積電 ESG 合作提案簡報 V2.0 (16:9)"""
    ...
    top_bar.fill.solid()
    top_bar.fill.fore_color.rgb = TSMC_RED
    ...
    partner_frame.text = "台積電慈善基金會 × 新竹市政府"
    ...
    for i, (num, title, desc) in enumerate(agenda_items):
        ...
        circle.fill.solid()
        circle.fill.fore_color.rgb = TSMC_RED
    ...
    for i, (emoji, title, metric, metric_desc, impact) in enumerate(challenges):
        ...
        metric_tf.text_frame.paragraphs[0].font.color.rgb = TSMC_RED
    ...
    summary_para.font.color.rgb = TSMC_RED
...
需要托育、親職教育"""
    ...
    data1_num.text_frame.paragraphs[0].font.color.rgb = TSMC_RED
    ...
    step1.fill.solid()
    step1.fill.fore_color.rgb = TSMC_RED
    ...
    step4_tf.text = "③ 台積電獲得：留任率+5% | ESG評級↑ | SROI 1:2.04"
    ...
    floors = [
        ...
        ("2F", "公共托嬰中心", "700m²", "45人（台積優先）", RGBColor(255, 183, 77)),
        ("1F", "長照日照中心", "800m²", "55人（台積優先）", RGBColor(149, 117, 205)),
        ("B1", "停車場+設備", "600m²", "30車位（台積保留10）", DARK_GRAY),
    ]
    ...
    title_frame = title_box.text_frame
    title_frame.text = "台積電投資方案 - 三年期合作計畫"
    title_para = title_frame.paragraphs[0]
    title_para.font.size = Pt(40)
    title_para.font.bold = True
    title_para.font.color.rgb = NAVY_BLUE
    ...
    for i, (year, amount, phase) in enumerate(zip(years, amounts, phases)):
        ...
        year_tf.text_frame.paragraphs[0].font.color.rgb = TSMC_RED
    ...
    cta_box.fill.solid()
    cta_box.fill.fore_color.rgb = TSMC_RED
    ...
    # 保存簡報
    filename = "台積電ESG合作提案_V2_16x9_2025.pptx"
    prs.save(filename)
